...
if args.policy == 'boltzmann_temperature':
    if args.exponential_decay_rate == 1:
        epsilon_fn = tf.keras.optimizers.schedules.PolynomialDecay(
                        initial_learning_rate=args.boltzmann_temperature_start*2, # multiply by 2 since we start after half of iterations 
...
collect_driver1 = dynamic_step_driver.DynamicStepDriver(
    train_env,
    train_policy,
    observers=[replay_buffer_observer, ResetPolicy(train_policy)] + train_metrics,
    num_steps=1)

collect_driver2 = dynamic_step_driver.DynamicStepDriver(
    train_env,
...
collect_driver2.run = common.function(collect_driver2.run)
...
for seasons_batch in range(args.train_seasons // args.plot_interval):
    ...
    if seasons_batch < (args.train_seasons // args.plot_interval // 2):
        collect_policy = train_policy
        collect_driver = collect_driver1
    else:
        collect_policy = agent.collect_policy
        collect_driver = collect_driver2
    
    print(f'Start training for batch of seasons {seasons_batch+1}/{args.train_seasons // args.plot_interval}')
    train_agent(args.plot_interval * 100, collect_policy, collect_driver)
    ...
    for i in range(args.sample_seasons_for_plots):
        ...
        plot_reward_per_period(dpc_game.competition_results_df, competition_id, selling_season, run)
    ...
    for i in range(args.sample_seasons_for_plots):
        ...
        plot_reward_per_period(dpc_game.competition_results_df, competition_id, selling_season, run, True)
