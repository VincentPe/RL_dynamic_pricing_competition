...
def plot_reward_per_period(data, competition_id, selling_season, run, eval_env=False):
    # Create temp dataset for competition id and season
    comp_data = data[
        (data['selling_season'] == selling_season) & 
        (data['competition_id'] == competition_id)
    ...
    plot_ = sns.pointplot(x=comp_data['selling_period'].astype(int), y=comp_data['reward'])
    plt.title(f"Total reward: {comp_data['reward'].sum()}")
    plt.grid()
    ...
    if run is not None:
        if eval_env:
            run.log_image(f'trajectory_plots/eval_reward_plot_selling_season_{int(selling_season)}', plot=plt)
        else:
            run.log_image(f'trajectory_plots/train_reward_plot_selling_season_{int(selling_season)}', plot=plt)
