...
for seasons_batch in range(args.train_seasons // args.plot_interval):
    ...
    train_agent(args.plot_interval * 100, train_policy)
    print(f'Save model {seasons_batch+1}/{args.train_seasons // args.plot_interval}')
    train_checkpointer.save(train_step_counter)
    
    for i in range(args.sample_seasons_for_plots):
        
        plot_qlogits_heatmap(qpolicy, eval_env, seasons_batch+1, i, run)
        
        latest_comp_results = dpc_game.competition_results_df['selling_season'].unique()[-11:-6]
