...
environment = AirlineEnvironment(dpc_game, simulator, num_features, num_actions, args.discount, 
                                 ...
                                 args.early_termination_penalty, args.price_diff_penalty, args.stock_remainder_penalty)
...
for seasons_batch in range(args.train_seasons // args.plot_interval):
    ...
    print(f'Start training for batch of seasons {seasons_batch}/{args.train_seasons // args.plot_interval}')
    train_agent(args.plot_interval * 100)
    print(f'Save model {seasons_batch}/{args.train_seasons // args.plot_interval}')
    train_checkpointer.save(train_step_counter)
    
    for i in range(args.sample_seasons_for_plots):
        
        selling_season = dpc_game.competition_results_df['selling_season'].max() - i
...
eval_avg_return = compute_avg_return(eval_env, dpc_game, agent.collect_policy, args.evaluation_nr_seasons)
run.log('Evaluation_reward', eval_avg_return)
run.parent.log('Evaluation_reward', eval_avg_return)

for i in range(args.sample_seasons_for_plots):
        
    selling_season = dpc_game.competition_results_df['selling_season'].max() - i -1
    competition_id = f'dqnagent{str(selling_season)}'
