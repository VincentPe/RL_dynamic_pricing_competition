...
def plot_price_and_loadfactor(data, competition_id, selling_season, run, eval_env=False):
    ...
    if run is not None:
        if eval_env:
            run.log_image(f'trajectory_plots/eval_trajectory_plot_selling_season_{selling_season}', plot=plt)
        else:
            run.log_image(f'trajectory_plots/train_trajectory_plot_selling_season_{selling_season}', plot=plt)
