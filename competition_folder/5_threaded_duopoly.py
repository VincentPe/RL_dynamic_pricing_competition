...
def begin_load_tensorflow():
    global tf_thread
    tf_thread = threading.Thread(target=load_tensorflow)
    tf_thread.start()


def load_tensorflow():
    global tf, tf_py_environment, sequential, dqn_agent, TFUniformReplayBuffer, common, Trajectory, PolicyStep, Dense
    global DynamicPricingCompetition, AirlineEnvironment, ExternalSimulator, ts
    # print("start importing tf")
...
def initialize_everything():
    global dpc_game, simulator, environment, train_env, q_net, optimizer, train_step_counter, epsilon_fn, agent
    global replay_buffer, train_checkpointer, dataset, iterator
...
def p(
    ...
    """
    if current_selling_season == 1:
        if selling_period_in_current_season == 1:
            begin_load_tensorflow()

        if selling_period_in_current_season <= 10:
            time.sleep(1)
        price = 50
        information_dump = {}
        # print(f'returning price 50 for selling season 1, period {selling_period_in_current_season}')
        # if selling_period_in_current_season == 1:
        #     print(f'Time till first response: {datetime.datetime.now() - start}')
        return (price, information_dump)

    elif (current_selling_season == 2) & (selling_period_in_current_season == 1):
        ...
        if selling_period_in_current_season == 1:
            ...
            if dpc_game.selling_period < 100:
                last_demand = 80 - dpc_game.loadfactor
            else:
                last_demand = 1
            ...
            prev_step = ts.TimeStep(
                tf.constant(information_dump['prev_discount']),
                tf.constant(information_dump['prev_observation']),
                tf.constant(information_dump['prev_reward']),
                tf.constant(information_dump['prev_step_type'])
            )
